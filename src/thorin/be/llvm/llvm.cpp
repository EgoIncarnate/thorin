...
Continuation* CodeGen::emit_intrinsic(Continuation* continuation) {
    ...
    switch (callee->intrinsic()) {
        ...
        case Intrinsic::OpenCL2:   return runtime_->emit_host_code(*this, Runtime::OPENCL2_PLATFORM, ".cl", continuation);
...
void emit_llvm(World& world, int opt, bool debug) {
    ...
    Scope::for_each(world, [&] (const Scope& scope) {
        ...
        else if (continuation->is_passed_to_intrinsic(Intrinsic::OpenCL2))
             imported = import(opencl2, continuation)->as_continuation();
        else
            return;
